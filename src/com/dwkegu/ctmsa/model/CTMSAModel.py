...
class CTMSAModel:
    ...
    def updatePsi(self, doc, sentence):
        ...
        optPsi = sop.minimize(self.fPsi, x0=sentence.oldPsi, args=(doc, sentence), method='CG', jac=self.dfPsi,
                              tol=1e-5)
        for i in range(doc.K):
            sentence.psi[i] = optPsi.x[i]
        mathUtil.normVec2One(sentence.psi)
    ...
    def fPsi(self, psi, doc, sentence):
        ...
        for i in range(sentence.wordNum):
            for j in range(doc.K):
                tmp -= math.exp(psi[j] + sentence.oldOmega ** 2 / 2) * sentence.wordNum / sentence.oldZeta
